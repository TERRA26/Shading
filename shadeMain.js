...
function setUpPhong(program) {
    ...
    // Set up Phong lighting parameters
    // Ambient light - soft overall illumination
    gl.uniform3fv(program.ambientLight, [0.2, 0.2, 0.2]);

    // Light position in world coordinates (positioned to the upper right front)
    gl.uniform3fv(program.lightPosition, [2.0, 2.0, 3.0]);

    // Light color - white light
    gl.uniform3fv(program.lightColor, [1.0, 1.0, 1.0]);

    // Base color of the object - deep red (like in the reference image)
    gl.uniform3fv(program.baseColor, [0.6, 0.1, 0.1]);

    // Specular highlight color - white for shiny highlights
    gl.uniform3fv(program.specHighlightColor, [1.0, 1.0, 1.0]);

    // Phong coefficients
    gl.uniform1f(program.ka, 0.3);  // ambient coefficient
    gl.uniform1f(program.kd, 0.7);  // diffuse coefficient
    gl.uniform1f(program.ks, 0.8);  // specular coefficient
    gl.uniform1f(program.ke, 32.0); // specular exponent (shininess)
...
function setUpCamera(program) {
    ...
    glMatrix.mat4.perspective(projMatrix,
        glMatrix.glMatrix.toRadian(45),  // field of view in radians
        1.0,                              // aspect ratio (canvas is square)
    ...
    // set up your view
    // Create a view matrix - position camera to look at the sphere
    let viewMatrix = glMatrix.mat4.create();
    glMatrix.mat4.lookAt(viewMatrix,
        [0, 0, 3],    // camera position (eye) - back along z-axis
        [0, 0, 0],    // look at point (center) - origin where sphere is
        [0, 1, 0]     // up vector
    );
